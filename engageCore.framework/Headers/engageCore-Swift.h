...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="engageCore",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_PROTOCOL("_TtP10engageCore21CampaignEventDelegate_")
@protocol CampaignEventDelegate
@optional
/// <ul>
///   <li>
///     Tells the delegate that specified campaign is about to trigger.
///   </li>
///   <li>
///     This method gets called just before triggering the campaign
///   </li>
/// </ul>
/// \param campaign an Instance of EngageCampaign
///
///
/// returns:
/// A Bool value that decides whether to trigger the campaign or not
/// Return true to show the campaign to user
/// Return false to avoid showing campaign to user
- (BOOL)onCampaignTriggeredWithCampaign:(EngageCampaign * _Nonnull)campaign SWIFT_WARN_UNUSED_RESULT;
/// <ul>
///   <li>
///     Tells the delegate that user has interacted with campaign.
///   </li>
///   <li>
///     This method gets called after user finished the campaign workflow.
///   </li>
/// </ul>
/// \param campaign an Instance of EngageCampaign
///
- (void)onCampaignActionWithCampaign:(EngageCampaign * _Nonnull)campaign;
@end

typedef SWIFT_ENUM(NSInteger, CampaignType) {
/// Campaign covering the whole activity
  CampaignTypeCTA_PAGE_TAKEOVER = 0,
/// Campaign which is shown with the dimensions of a typical android dialog box
  CampaignTypeCTA_MODAL = 1,
/// Campaign shown as bar in the top of the activity
  CampaignTypeCTA_BAR = 2,
/// Campaign covering the whole activity with options to share content
  CampaignTypeCTA_SHARING = 3,
/// Campaign dialog with multiple choices
  CampaignTypeSURVEY_MULTIPLE_CHOICE = 4,
/// Campaign dialog with a text area where user can type their feedback
  CampaignTypeSURVEY_USER_FEEDBACK = 5,
/// Campaign dialog with rating stars for the user feedback
  CampaignTypeSURVEY_RATING = 6,
/// Default value
  CampaignTypeNONE = 7,
};
...
+ (void)shutDown;
/// User can register to campaign events.
/// For more details regarding the events supported refer CampaignEventDelegate interface
/// \param campaignEventDelegate an Object that implements CampaignEventDelegate protocol
///
+ (void)registerForCampaignEventsWithCampaignEventDelegate:(id <CampaignEventDelegate> _Nonnull)campaignEventDelegate;
...
@property (nonatomic) enum CampaignType type;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
typedef SWIFT_ENUM(NSInteger, REGISTRATIONSTATUS) {
/// Registered User
  REGISTRATIONSTATUSREGISTERED = 0,
/// Anonymous User
  REGISTRATIONSTATUSANONYMOUS = 1,
/// Subscribed User
  REGISTRATIONSTATUSSUBSCRIBED = 2,
/// Default value
  REGISTRATIONSTATUSNONE = 3,
};
...
@property (nonatomic) enum REGISTRATIONSTATUS status;
...
/// \param age An Int value representing age of user.
///
/// \param gender An enum value of GENDER supports values (.MALE,.FEMALE,.OTHER)
///
/// \param country String representing user country name
///
/// \param city String representing user city name
///
/// \param network String representing user network
///
/// \param name String name of user
///
/// \param status String value representing registration Status of user.
///
/// \param id String value representing UserID of user.
///
/// \param customTraits A [String : String] dictionary representing custom user prfile informations.
///
- (nonnull instancetype)initWithId:(NSString * _Nullable)id name:(NSString * _Nullable)name email:(NSString * _Nullable)email gender:(enum GENDER)gender age:(NSNumber * _Nullable)age status:(enum REGISTRATIONSTATUS)status phoneNumber:(NSString * _Nullable)phoneNumber language:(NSString * _Nullable)language network:(NSString * _Nullable)network customTraits:(NSDictionary<NSString *, NSString *> * _Nullable)customTraits OBJC_DESIGNATED_INITIALIZER;
